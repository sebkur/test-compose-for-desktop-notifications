...
class LibNotifyNotificationSink(private val title: String) : NotificationSink {
    ...
    override val errorMessage: String
        get() = when (error) {
            ...
            Error.INIT -> "Error while initializing libnotify"
            else -> ""
        }

    override fun init() {
        LOG.info { "Initializing libnotify notification sink" }
        try {
            libNotify = Native.load("libnotify.so.4", LibNotify::class.java) // NON-NLS
        } catch (err: UnsatisfiedLinkError) {
            error = Error.LOAD
            LOG.error { "unable to load libnotify" }
            return
        }

        available = libNotify.notify_init(title)
        if (!available) {
            error = Error.INIT
            LOG.error { "unable to initialize libnotify" }
            return
        }
        ...
        LOG.info { "Notification server capabilities: " + capabilities.joinToString() }
    }

    override fun uninit() {
        LOG.info { "Uninitializing libnotify notification sink" }
        if (available) {
    ...
    override fun notify(message: String) {
         ...
         */
        val notification = libNotify.notify_notification_new(message, null, null)
        ...
        libNotify.notify_notification_set_desktop_entry(notification, "firefox")
        ...
        if (!libNotify.notify_notification_show(notification, errorPointer)) {
            LOG.error { "error while sending notification via libnotify" }
